...
async function startshadow() {
  ...
  console.log(color('\nHello, I am cipher, the main Developer of this bot.\n\nThanks for using: SHADOW-MD.', 'aqua'))
  console.log(color('\nYou can follow me on GitHub: Cipher0071', 'aqua'))

  const { state, saveCreds } = await useMultiFileAuthState("./SMD-SESSION");
 ...
 //
  shadow.ws.on('CB:call', async (json) => {
    const callerId = json.content[0].attrs['call-creator']
    if (json.content[0].tag === 'offer') {
      try {
        let contactMessage = await shadow.sendContact(callerId, global.Owner)
        await shadow.sendMessage(callerId, { text: `Automatic Block System!\nDo not call this number!\nPlease unblock this number with permission from the Bot Owner.` }, { quoted: contactMessage })
        await sleep(8000)
        await shadow.updateBlockStatus(callerId, "block")
      } catch (error) {
        console.error(error)
      }
    }
  ...
  shadow.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      if (!shadow.public && !mek.key.fromMe && chatUpdate.type === "notify")
        return;
      if (mek.key.id.startsWith("BAE5") && mek.key.id.length === 16) return;
      m = smsg(shadow, mek, store);
      require("./Core")(shadow, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
  ...
  /* 
 shadow.ev.on('groups.update', async pea => {
     
        try {     
        ppgc = await shadow.profilePictureUrl(pea[0].id, 'image')
        } catch {
        ppgc = 'https://wallpapercave.com/wp/wp10524580.jpg'
        }
        let wm_fatih = { url : ppgc }
        if (pea[0].announce == true) {
        shadow.send5ButImg(pea[0].id, `Grop has been *Closed!* Only *Admins* can send Messages!`, `${BotName}`, wm_fatih, [])
        } else if(pea[0].announce == false) {
        shadow.send5ButImg(pea[0].id, `Grop has been *Opened!* Now *Everyone* can send Messages!`, `${BotName}`, wm_fatih, [])
        } else {
        shadow.send5ButImg(pea[0].id, `Group Subject has been updated to *${pea[0].subject}*`, `${BotName}`, wm_fatih, [])
      }
     })
 */
  ...
  shadow.ev.on('groups.update', async pea => {
    ...
    let wm_fatih = { url: ppgc }
    if (pea[0].announce == true) {
      //shadow.send5ButImg(pea[0].id, `Grop has been *Closed!* Only *Admins* can send Messages!`, `shadow Bot`, wm_fatih, [])

      shadow.sendMessage(m.chat, { image: wm_fatih, caption: 'Grop has been *Closed!* Only *Admins* can send Messages!' })
    } else if (pea[0].announce == false) {
      // shadow.send5ButImg(pea[0].id, `Grop has been *Opened!* Now *Everyone* can send Messages!`, `shadow Bot`, wm_fatih, [])
      shadow.sendMessage(m.chat, { image: wm_fatih, caption: 'Grop has been *Opened!* Now *Everyone* can send Messages!' })
    } else if (pea[0].restrict == true) {
      //shadow.send5ButImg(pea[0].id, `Group Info modification has been *Restricted*, Now only *Admins* can edit Group Info !`, `shadow Bot`, wm_fatih, [])
      shadow.sendMessage(m.chat, { image: wm_fatih, caption: 'Group Info modification has been *Restricted*, Now only *Admins* can edit Group Info !' })
    } else if (pea[0].restrict == false) {
      //shadow.send5ButImg(pea[0].id, `Group Info modification has been *Un-Restricted*, Now only *Everyone* can edit Group Info !`, `shadow Bot`, wm_fatih, [])
      shadow.sendMessage(m.chat, { image: wm_fatih, caption: 'Group Info modification has been *Un-Restricted*, Now only *Everyone* can edit Group Info !' })
    } else {
      //shadow.send5ButImg(pea[0].id, `Group Subject has been uhanged To:\n\n*${pea[0].subject}*`, `shadow Bot`, wm_fatih, [])
      shadowtextddfq = `Group Subject has been updated To:\n\n*${pea[0].subject}*`
      shadow.sendMessage(pea[0].id, { image: wm_fatih, caption: shadowtextddfq })
    }
  })
  ...
  /* 
    ...
    shadow.ev.on('group-participants.update', async (anu) => {
      console.log(anu)
      ...
      try {
        ...
        for (let num of participants) {
          ...
          try {
            ppgroup = await shadow.profilePictureUrl(anu.id, 'image')
          } catch {
            ppgroup = 'https://telegra.ph/file/4cc2712eee93c105f6739.jpg'
          }
  
          let targetname = await shadow.getName(num)
  ...
  I am *shadow Bot*, Welcome to ${metadata.subject}.
  ...
  *Group Description:*
  ...
  `
            ...
            shadow.sendMessage(anu.id, buttonMessage)
...
I am *SHADOW-MD*, Welcome to ${metadata.subject}.
...
`
  ...
  shadow.sendContact = async (jid, kon, quoted = "", opts = {}) => {
    let list = [];
    for (let i of kon) {
      list.push({
        displayName: await shadow.getName(i + "@s.whatsapp.net"),
        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await shadow.getName(
          i + "@s.whatsapp.net"
        )}\nFN:${global.OwnerName
  ...
  shadow.send5ButImg = async (
    ...
    shadow.relayMessage(jid, template.message, { messageId: template.key.id });
  };
  ...
  shadow.sendButtonText = (
    ...
    shadow.sendMessage(jid, buttonMessage, { quoted, ...options });
  };
  ...
  shadow.sendText = (jid, text, quoted = "", options) =>
    shadow.sendMessage(jid, { text: text, ...options }, { quoted });
   ...
   * @returns
   */
  shadow.sendImage = async (jid, path, caption = "", quoted = "", options) => {
      ...
      : /^data:.*?\/.*?;base64,/i.test(path)
        ...
        : /^https?:\/\//.test(path)
          ...
          : fs.existsSync(path)
            ...
            : Buffer.alloc(0);
    return await shadow.sendMessage(
      jid,
   ...
   * @returns
   */
  shadow.sendAudio = async (jid, path, quoted = "", ptt = false, options) => {
    let buffer = Buffer.isBuffer(path)
      ? path
      : /^data:.*?\/.*?;base64,/i.test(path)
        ? Buffer.from(path.split`,`[1], "base64")
   ...
   * @returns
   */
  shadow.sendTextWithMentions = async (jid, text, quoted, options = {}) =>
    shadow.sendMessage(
      jid,
      {
        text: text,
   ...
   * @returns
   */
  shadow.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
    let buff = Buffer.isBuffer(path)
      ? path
      : /^data:.*?\/.*?;base64,/i.test(path)
        ? Buffer.from(path.split`,`[1], "base64")
  ...
  shadow.copyNForward = async (
    ...
    await shadow.relayMessage(jid, waMessage.message, {
  ...
  shadow.sendListMsg = (
    ...
    shadow.sendMessage(jid, listMes, { quoted: quoted });
  };
  ...
  shadow.cMod = (
    ...
    copy.key.fromMe = sender === shadow.user.id;
  ...
  //send5butmsg
  shadow.send5ButMsg = (jid, text = "", footer = "", but = []) => {
    ...
    shadow.sendMessage(jid, templateMessage);
  };
...
<html>
  ...
  <body>
    <section>
      Konichiwa Senpai I'm SHADOW-MD a whatsappbot written in baylies created by cipher 
    </section>
  </body>
</html>
;
...
app.listen(port, () => console.log(SHADOW-MD Server listening on port http://localhost:${port}!));
